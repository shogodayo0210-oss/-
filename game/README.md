...
11. **高いユニットほど再出撃までが長い。** 比ではなく順序で縛る ――
...
**32体。** ティアのコスト帯は重ならない（B 75〜300 / A 400〜950 / S 1100〜2500）。
...
レベルを1段上げると、**貯まる上限**と**貯まる速度**の**両方が少しずつ**上がる。
上げるかどうかはプレイヤーが選ぶ。

| レベル | 貯まる上限 | 毎秒 | 次への費用 | ここで出せるようになるもの |
| --- | ---: | ---: | ---: | --- |
| **Lv1** | 500 | 60 | 400 | 兵卒・斥候鼠・猟犬・盾兵… |
| **Lv2** | 700 | 72 | 600 | 重装・呪い手・土人形・双剣 |
| **Lv3** | 950 | 88 | 800 | 火炎兵・騎兵・薙ぎ払い・狂戦士… |
| **Lv4** | 1,250 | 105 | 1,100 | 臼砲・砲手 |
| **Lv5** | 1,600 | 128 | 1,400 | 大呪術・攻城弩・影討ち・亡霊将 |
| **Lv6** | 2,000 | 155 | 1,800 | 聖騎士・攻城櫓 |
| **Lv7** | 2,500 | 185 | 2,300 | 巨獣・巨兵 |
| **Lv8** | 3,100 | 225 | ― | **（無し。ここから先は純粋に経済）** |

開始は 300。**レベルアップには6秒かかり、その間は出撃も呪文も切り札も止まる。**
ここが殴りどころになる。

**1段あたりの伸びは控えめ**（毎秒 ×1.21・上限 ×1.3 前後）だが、
8段ぶんで毎秒は 3.75倍、上限は 6.2倍になる。
**Lv1〜Lv7 はどの段でも必ず何かが解禁され、Lv8だけが純粋な経済**という形にしてある。
...
コストの階段は、にゃんこ大戦争にならって**きりのいい数字**で刻んである ――
B帯は25刻み（75〜300）、A帯は50刻み（400〜950）、S帯は100刻み（1,100〜2,500）。
最安と最高で**33倍**の開きがある。
...
いったん待ち行列に入り、次のtickの頭で使われる（`play/human.py`）。
