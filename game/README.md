...
工程表（`ROADMAP.md`）第1期の1〜3まで ―― 描画・出撃ボタン・資金バーと拠点HP。
...
操作は3つだけ。**クリックか数字キーで出撃**、Space で一時停止、R でやり直し。
...
カード・切り札・見切り・成長・編成フェーズ（工程表 第2期）。
相手だけは `policy.py` が動かすので、この段階では勝てなくてよい。
ここで見たいのは**触れるか**と**読めるか**であって、勝敗ではない。
