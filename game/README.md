...
1v1 対戦の骨格・呪文カード・**メインキャラクター特典**の設計。
...
- [3. 呪文カード](#3-呪文カード)
- [4. メインキャラクターと特典](#4-メインキャラクターと特典本題)
- [5. 公平性と実装](#5-公平性と実装)
- [6. 調整プロセス](#6-調整プロセス)
- [7. 作る順番](#7-作る順番)
- [8. 未決の論点](#8-未決の論点)
...
```mermaid
flowchart LR
  A["① マッチング<br/>メイン相互公開"] --> B["② 編成 45s<br/>6体選択＋カード3枚<br/>（どちらも非公開）"]
...
**メインキャラクター**は、編成する8体とは別に1人1体だけ選ぶ枠。
戦闘には出ず、特典だけを試合に持ち込む。
...
戦場に出ないのに「メインキャラクター」と呼ぶのは据わりが悪いので、
**セコンドとして画面の端に立たせる**のを推奨する。アイドル1種と、特典が動いた
時のリアクション1種で足りる。存在感が出るうえに、戦闘用のモーション一式を
作らずに済む。
...
1. **相手にも見える。** メインキャラクターはマッチング時に相互公開。持っている特典も公開。
   隠れた非対称性を作らない（隠すと情報系特典が一方的に強くなる）。
2. **予算は全メイン同じ。** 合計 9〜10pt（`data/perks.json`）。
...
### 4.4 メインキャラクター例（`data/mains.json`）

**看板**が一目で言えることを条件にしている。残りは味付けで、覚えなくていい。
...
`tools/validate.py` が予算・カテゴリ上限・参照整合・看板の有無を機械的に検査する。
メインや特典を足したら必ず通す。
...
| 指標 | 目標 | 外れた時 |
| --- | --- | --- |
| メイン別勝率 | 50% ±1.5% | pt を上下、または効果量を調整 |
| メイン採用率 | 5〜25% | 5%未満＝弱い / 25%超＝一強 |
...
- **メインキャラクターの入手方法。** 課金で買えると 4.1 の原則1・2があっても
  「金で情報を買う」構図になる。全メインを無料開放し、
  課金は見た目に寄せるのが安全。
- **メインを戦闘キャラの中から選ぶ統合案。** 本書は「別枠・戦場に出ない」で
  書いたが、*戦闘キャラ全員が特性を1つ持っていて、メインに指定した時だけ
  発動する*という作り方もある。メイン専用のキャラを別に用意せずに済み（制作が軽い）、
  「メインに指定＝そのキャラが確実に編成に入る＋特典が付く」で3つの決定が1つに畳める。
  代わりに、公開されたメインから手札が1体バレる。この副作用は面白いと思うが、
  「キャラとは別に」というご意向とは別案なので論点として置く。
