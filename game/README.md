...
**(2) 8枠は「持ち駒」ではなく「種類」。** 同じユニットを何度でも出せる。
だから抽選で来た2種類も、*出さないという選択肢がある*。
引き事故が敗北に直結しない。

**(3) 資金は出撃とカードで共用。** 今ユニットを出すか、貯めて大技を撃つか。
この一本の択が試合の背骨になる。財布を分けると、両方を無条件に使えて
択がなくなる。
...
| 数字 | 意味 |
| --- | --- |
| **体力** | ユニット自身の耐久。前に居られる時間 |
| **攻撃力** | 1発の威力 |
| **攻撃間隔** | 手数。短いほど攻撃頻度が高い |
| **攻撃範囲** | 当たる**帯**。`[手前, 奥]` の2つの数字で持つ（2.1） |
| **移動速度** | 前進の速さ。**前線がどこで作られるか**を決める（2.2） |
...
4. **ティアのコスト帯は重ならない。** 3.2 の鏡像抽選の土台。
5. **再出撃CDは コスト÷25 秒が目安**（±60%）。高いユニットほど連打できない。

### 2.4 見本（`data/characters.json`）

実際は20体以上要る。これは型が一通り揃うだけの見本。

| ユニット | T | 資金 | CD | 体力 | 攻撃力 | 間隔 | DPS | 攻撃範囲 | 速度 | 役割 |
| --- | :-: | ---: | ---: | ---: | ---: | ---: | ---: | --- | ---: | --- |
| 兵卒 | B | 60 | 3s | 400 | 60 | 1.2s | 50 | `[0,12]` 単体 | 8 | 数で押す。一番安い壁 |
| 槍兵 | B | 120 | 5s | 700 | 110 | 1.4s | 79 | `[0,25]` 単体 | 7 | 一歩外から刺す |
| 弓手 | A | 200 | 8s | 450 | 130 | 1.7s | 77 | `[0,90]` 単体 | 4 | 遠距離単体。壁が要る |
| 重装 | A | 260 | 10s | 3000 | 70 | 2.0s | 35 | `[0,14]` 単体 | 4 | 壁役。時間を買う |
| 双剣 | A | 300 | 12s | 900 | 200 | 0.8s | 250 | `[0,13]` 単体 | 9 | 最も手数が多い。紙 |
| 薙ぎ払い | A | 340 | 14s | 1200 | 180 | 2.0s | 90 | `[0,45]` **範囲** | 5 | **前線範囲**。群れに強い |
| 臼砲 | S | 400 | 16s | 500 | 480 | 3.2s | 150 | `[70,150]` **範囲** | 3 | **後方範囲**。懐に入られたら無力 |
| 砲手 | S | 420 | 18s | 600 | 480 | 3.5s | 137 | `[0,130]` 範囲 | 3 | 死角なしの遠距離範囲 |
| 巨兵 | S | 600 | 25s | 6000 | 350 | 2.4s | 146 | `[0,20]` 範囲 | 3 | 最上位。遅く、資金が空になる |

特殊効果（対拠点特効、ノックバックなど）はまだ足していない。
**素の6つで役割が立ってから**にしたい（11章）。
...
| 開始 | 200 |
| 自然増 | 毎秒 +40 |
| 撃破報酬 | 倒した敵ユニットのコスト × 0.4 |

出撃にもカードにも切り札にも、この一本から払う。
**巨兵（600）を出すと、しばらくカードが撃てない。** それでいい。
...
| 使用 | 資金を消費。使い切り |
| 詠唱 | 0.4〜1.0秒。**明確な予兆つき**。短縮しても下限 0.35秒（7.5） |
| 構築制限 | デッキ合計コスト **1000以下**（大技3枚積みの禁止） |

「残枚数だけ公開」がこのシステムの肝。**何を持っているかは分からないが、
あと何回来るかは分かる**。

### 5.2 サンプル（`data/cards.json`）

| カード | 資金 | 詠唱 | 効果 |
| --- | ---: | ---: | --- |
| 火柱 | 250 | 0.6s | 指定地点に炎柱。範囲中ダメージ |
| 治癒の光 | 200 | 0.8s | 自拠点のHPを10%回復 |
| 時縛 | 400 | 0.7s | 敵ユニット全体を1.2秒停止 |
| 影分身 | 300 | 0.5s | 自軍最前列の分身を8秒間 |
| 反射壁 | 275 | 0.4s | 自陣前方に2秒の反射壁 |
| 総攻撃 | 450 | 1.0s | 自軍全ユニットの攻撃力+50%を8秒 |

強いカードほど詠唱が長い＝止められやすい。**強さは長い予兆で払わせる。**
...
- **資金の成長を選べるようにするか。** にゃんこ大戦争の「働きネコ」に当たる、
  資金の上限や増加速度を試合中に上げる仕組み。入れると
  「今出すか、育てるか」の択が序盤に生まれて厚みが出るが、
  カード・切り札と合わせて択が3つになり、初見には多すぎるかもしれない。
...
- **ユニットの特殊効果をいつ足すか。** 2章は6つの数字だけで組んである。
  「対拠点特効」「ノックバック」などを足すのは、素の数字で役割が
  立ってからにしたい。
