...
序盤は安いユニットを出した側が取り、終盤は育てた側が取り返す、という形。
...
**まだ直っていないもの。** *中盤（50%時点）に押されていた側が一度も勝っていない*
（0/10）。前半で決まりきっている。押し込まれてから開く札（5.4）は入れたが、
まだ届いていない ―― `balance.py` がこれを NG として出すので、
ここから先はその数字を見ながら特性とキャラの数字で詰める。
