...
| --- | --- |
| レーン240m | **1/12**（8.3%） |
| 画面（レーンを1120pxで描く） | **93px** ＝ 目盛り40mの半分 |
| キャラの絵 | **巨兵で0.9体ぶん**（体幅104px）／**兵卒だと2.7体ぶん**（34px） |
| 接近戦の射程（12〜20m） | **射程より広い**。殴り返せる距離から完全に外れる |
| 遠距離の射程（68〜80m） | 射程の25〜29%。**まだ届く** |

**絵の大きさがコストに比例する**ようになったので、20mの見え方はユニットで
割れる ―― 小さい削り役にとっては体3つぶんの後退で、巨兵にとっては
*自分の体の幅にも足りない*。**下がって痛いのは後者のほう**なのに、
画面で大きく動くのは前者、という向きが逆の関係がここにある。
...
**絵にするときの注意。** 移動距離だけでは、見た目に事件が起きたように
見えない ―― **同じ93pxが、巨兵には7.1秒で斥候鼠には2.1秒**だから。
画面上の見た目が同じで、払うものが3倍以上ちがう。
硬直0.4秒のあいだ判定が消えることと合わせて、**画面側で
「いま下がった」を分からせる**必要がある。

**そこは画面側で解いた**（14章）。距離を見せるのをやめて、
**どこから下がったか**（跡）と**判定が消えていること**（半透明・足元の輪郭・
「無敵」）を描く。跡は下がった20mぶんの長さで残るので、
*通り過ぎた動き*ではなく**残る幅**として読める。
...
- ~~**引き分けが10%残っている**~~ ―― **落ちた**（12.7）。53% → 10% →
  **1〜2%**。最後のひと押しは狙って直したものではなく、*雷の伸びが2倍速だった
  バグ*の修正と、遠距離の攻撃間隔を30%緩めたこと。決着率は98〜99%で、
  「雷が降ってもなお前線が立て直され続ける試合」はほぼ残っていない。
- **試合が長い。** 時間制限を外したので平均364〜414秒（前は260秒）。
  早送り（14章）で見る側は救ったが、*人が遊ぶ長さとして正しいか*は
  まだ確かめていない。**ここが塊A-4で答えの出る最大の論点**
  ―― 4本の線のうち「退屈して速度を上げた」が直接これを測っている（15.1）。
...
**まだ借りていないもの。** 並び順は「引き分けへの効きが大きいと思う順」で
付けたものだが、**その引き分けはもう1〜2%まで落ちた**（12.11）。
つまりこの順位の根拠は消えている ―― いま足すとしたら理由は膠着ではなく
**読み合いの厚み**のほうで、1（手札）と7（出現ラグ）がそちら側に効く。
残りは、解く相手がもう居ない。
...
*①* コスト1の無限供給が消える（膠着への直撃 ―― ただし膠着自体はもう
1〜2%なので、いまこれは**理由としては弱い**・12.11）、
...
| | 前（時間切れあり・見本3種） | 雷を入れた直後 | **いま**（12.10） |
| --- | ---: | ---: | ---: |
| 決着率 | 47% | 90% | **98〜99%** |
| 引き分け | 53% | 10% | **1〜2%** |
| 先制→勝ちの一致率 | ― | 26% | **27〜30%**（序盤の優勢は決めない） |
| 雷の直前→勝ちの一致率 | ― | 78% | **77〜79%**（盤面は効くが、絶対ではない） |
| 雷まで届いた試合 | ― | 94%（中央 5回） | **92〜93%**（中央 6回） |
...
**この表は見本編成3種で測ったもの**（`--rosters` を付けていない）。
12.7 ② がまさにそこで転んでいる ―― 見本3種だけだと盤面がいつも同じ形に
落ち着き、引き分けが実態より多く出る。だから上の「39%」は
*遠距離を緩めた後の引き分け率*ではなく、*見本3種で測ったときの引き分け率*。
変更の前後を同じ測り方で比べているので差分としては読めるが、
**水準としては読めない**。実際の水準は次節。

### 12.11 測り直したら、引き分けはもう残っていなかった

**data を触ったので測り直した**（遠距離の攻撃間隔 +30%・雷の伸びのバグ修正・
安全弁1000秒）。編成を振る測り方で、2回。

| | 雷を入れた直後 | `--rosters 4`<br>48試合 | `--rosters 6`<br>108試合 |
| --- | ---: | ---: | ---: |
| 決着率（拠点撃破） | 90% | **98%** | **99%** |
| **引き分け** | **10%** | **2%** | **1%** |
| 先制→勝ちの一致率（36秒） | 26% | 30% | 27% |
| 雷の直前→勝ちの一致率（180秒） | 78% | 79% | 77% |
| 逆転率 | ― | 21% | 23% |
| 与ダメージ（多い側の平均） | 90% | 98% | 99% |
| 平均の長さ | 400秒 | 414秒 | 364秒 |
| 雷まで届いた試合 | 94%（中央5回） | 92%（中央6回） | 93%（中央6回） |

**11.1 に「残り10%」として挙げていた論点は、もう無い。**
面白いのは、それを潰したのが狙った調整ではなかったこと ――
**雷の伸びが2倍速だったバグ**の修正が効いている。1.2 で「強い雷ほど決着しない」
と測ってあったとおりで、*意図より強い雷が前進そのものを禁止していた*。
バグの向きが、先に測ってあった法則の向きと一致していた。

**逆転率21〜23%** は雷を入れる前（11%・12.5）の倍。
*押されている側にまだ手がある*という形は、決着率が上がっても保たれている。

**残っている穴は1つだけで、それは動いていない** ――
解禁される札（5.4）の使用は**両方の測定で0回**。11.1 に挙げたとおり、
方針が呪文の3回を種族呪文で前半に使い切ってしまう。これは盤面ではなく
`policy.py` の側の課題で、*人が遊べば違う*可能性がある（塊A-4で分かる）。
...
問わず、画面上の高さ（ユニット96px基準・拠点128px相当）に正規化して表示する。
ユニットの96pxは基準であって固定ではなく、**コストの順位（percentile）から
0.7〜1.3倍の縮尺が自動でかかる**（最安67px相当〜最高額125px相当。2.6と3章
「コストが高いほど大きい」の規約を実際の描画にも適用したもの）。
...
実測の平均は364〜414秒（12.11）―― 見ているだけの時間を飛ばせないと、試遊も実測も続かない。
...
| 体力の棒に入った縦線 | **ノックバックの区切り**（2.11）。跨がせれば下がる |
| 水色の跡と、その先の縦棒 | **さっきまで居た位置**。跡の長さが後退した20m |
| 半透明の本体＋足元が輪郭だけ＋「無敵」 | **当たり判定が消えている**（`Fighter.hittable`） |
...
### 14.1 ノックバックを、目で分かるようにした

**症状。** 後退の20mは画面で93px。**巨兵にとっては自分の体幅（104px）にも
足りず**、それでいて歩いて戻すのに7.1秒かかる（2.11）。斥候鼠なら同じ93pxが
2.1秒 ―― **見た目が同じで、払うものが3倍以上ちがう**。
硬直中は判定が消える（殴られず、すり抜けられる）のに、それも出ていなかった。

**直したのは、距離を見せようとするのをやめたこと。** 93pxをどう強調しても
93pxは93pxなので、代わりに**残るもの**を描く ―― 3つとも `Battle` を読むだけで、
View はフレームをまたぐ状態を持たない（起点は `x + 向き × 後退距離` で出る。
硬直中は動かないので、この値は硬直のあいだ固定される）。

| 出すもの | 何を言っているか |
| --- | --- |
| **水色の跡＋起点の縦棒** | 「いま下がった」。**跡は後退した20mぶんの幅で残る** ―― 通り過ぎた動きではなく、読める形になる |
| **半透明の本体＋足元が輪郭だけ＋「無敵」** | 「いま的ではない」。塗り＝ここに立っている的／輪郭だけ＝居るが的ではない |
| **体力の棒に入った縦線** | 後退は「体力を kb 個に割った区切りを跨いだ瞬間」（2.11）。この線は*次にどこで下がるか*の予定表 |

色は盤面で使っていなかった水色に寄せた ―― 金＝振りかぶり「来るぞ」、
赤＝後隙「いまなら通る」に対して、**水色＝「いま的ではない」**。

**体力の棒の線は、実効値で引く。** 死守・踏破（ノックバック×0）が効いている
あいだは線が消えるので、「いま押し戻せない相手」が形で分かる。線が見えるのは
体力の残っている側だけなので、**見えている線の数がそのまま「あと何回押し戻せるか」**
になる ―― 重い一撃が区切りを2本まとめて食う（＝そこから先は下がらなくなる）のも、
線の減り方で読める。
...
**見切り（0.3秒）を押す口が無い**（`ROADMAP.md` 塊B-2）。engine には有るが、
試遊版のアバターがその特典を持っていない。
...
| `select.js` | 戦闘の前に置く編成画面（アバター → 出撃ユニット → 切り札・持ち込み呪文） |
| `main.js` | `play/__main__.py` の移植（実時間の溜め込みと入力）＋ 編成画面との橋渡し |
...
| 出撃できる種類 | `--unit` で自由 | **編成画面で自由に選ぶ**（最大8種。3章の6選択＋2抽選の簡略版 ―― commit-reveal も鏡像抽選もまだやらない） |
| アバター・切り札・持ち込み呪文 | `--avatar` / `--trump` / `--brought` で自由 | 同じく編成画面で選ぶ |
| 相手 | 3方針 ＋ `--enemy-roster full` | 3方針（持ち物は**自分が選んだ編成とまったく同じ**） |
| 呪文の並び | その場で引く | **40試合ぶんを焼き込み済み**（それ以降は先頭に戻る。編成をどう選んでも同じ並びを使う） |
| 戦績 | 残らない | **その端末の中だけに残る**（どこにも送らない） |
| 塊A-4／A-5 の記録と判定 | 無し | **画面の中でやる**（15.1） |

盤面の中身は同じ ―― 一致試験がそれを保証している。

### 15.1 面白いかを、遊びながら数える

工程表の塊A-4／A-5（10試合遊んで判定する）を、画面の中で完結させた。
**紙とゲームを往復すると、たいてい記録のほうが落ちる**ので。

決着した直後に答える欄が出る。聞くのは**2つだけ** ―― 「次はこうしよう」と
思った回数と、最後まで結果が読めなかったか。残り2本（もう1回を押したか・
退屈して速度を上げたか）は**機械が数える**ので、答えなくても付く。

| 指標 | 誰が数えるか | 何を見ているか |
| --- | --- | --- |
| もう1回を押した | 機械 | 決着のあと、続けてもう1試合始めたか |
| 退屈して速度を上げた | 機械 | **試合中に**×1より速くしたか。決着後の早送りは数えない |
| 「次はこうしよう」と思った回数 | 人 | 0／1／2／3回以上 |
| 最後まで結果が読めなかった | 人 | はい／いいえ |

**線は `game/data/playtest.json` にあり、画面からは書き換えられない。**
工程表の表と同じ数字がそこに1つだけある ―― 数字と説明を両方持つと
必ずどちらかがズレるので（DPS を data に持たせないのと同じ理由）。
そして塊Aの「**先に決めておくのが肝**。遊んだ後に基準を作ると必ず自分に
甘くなる」を、仕組みの側で守るため。

10試合そろうまでは判定を出さず「あと何試合」とだけ出す。そろったら
**何本割れているか**と、塊Bに進んでよいかをそのまま書く。
1行のひとことも同じ欄で書けて、試合の一覧にそのまま並ぶ ――
数字より、後で読み返すのはこちら。
