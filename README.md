...
$ python3 -m unittest discover -s game/tests -t . # 64件
```
...
**気づいている粗さ。** 一撃で2段跨いでも下がるのは20m1回だけで、段だけ2つ
消費される。つまり*同じ総ダメージなら、重い一撃より手数のほうが押し込める*。
意図した設計ではないので、直すなら「跨いだ段数ぶん下がる」が素直。
