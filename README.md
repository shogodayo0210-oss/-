...
$ python3 -m unittest discover -s game/tests -t . # 57件
```
...
27. **値段が2倍以上違うなら、1体ずつなら高いほうが勝つ。**（2.6・12.6）
    死角持ちと壁は外す ―― 懐に入られたら負けるのが遠方範囲の設計で、
    壁は殴り勝つ道具ではない。
28. **種族呪文は1種族に1枚ずつ。自軍にかけるものに限る。**（5.5）
...
- **引き分けが53%あり、その全部が拠点無傷の0対0。** いまの一番大きな穴。
  内訳は資金の育った編成どうし（balanced と greed）に集中している。
  押されている側は自陣のすぐ前にコスト1を出し続けられるので、
  **安い1体が敵軍20体を止め続ける**（射程に敵が入ると全員その場で止まって殴るため）。
  前線起点の射抜き（2.1）を足して *壁ごと後ろを抜く* 手は作ったが、
  供給そのものが無限なので均衡は割れていない。答えの候補は 11.2 に挙げた。
...
いまは **261組すべてで高いほうが勝つ**。`validate.py` が毎回回す（1秒弱）。
