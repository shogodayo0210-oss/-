...
$ python3 -m unittest discover -s game/tests -t . # 76件
```
...
| 安全弁 | 900秒 | **700秒。** 雷を強くするのではなく、詰まった試合を早く切り上げる（1.3） |
...
`victory.hard_stop_sec`（**700秒**。導入時は900秒 → 1.3で詰めた）は
...
### 1.3 試合をスピーディーに ―― 安全弁は900秒から700秒へ
...
**そこで安全弁そのものを詰めた。** 75試合で、安全弁を動かしたときに
何が起きるかを測る（試合そのものは変えず、打ち切り時刻だけをずらす）：

| 安全弁 | 平均 | 拠点撃破 | 何らかの勝者あり | **真の引き分け** |
| ---: | ---: | ---: | ---: | ---: |
| 900秒（旧） | 380秒 | 99% | 100% | **0%** |
| **700秒（いま）** | 373秒 | 95% | 96% | **4%**（3件） |
| 600秒 | 362秒 | 87% | 87% | 13%（10件） |
| 500秒 | 346秒 | 80% | 83% | 17%（13件） |

**700秒で止めた。** 平均はほぼ動かない（380→373秒）―― 影響するのはもともと
稀な試合だけなので。代わりに最悪ケースの上限が**22%縮む**。真の引き分け
（拠点撃破どころか、時間切れの与ダメージ割合でも決まらない試合）は
0%→4%に増えるが、600秒まで詰めると13%に跳ねるので、そこでは止めない。
...
| 臼砲（22） vs 盾兵（2） | DPS103 対 12・射程・貫通・対拠点 | **盾兵11体で体力29700 対 560** |
...
「無敵」）を描く。移動そのものは78pxしか無いが、*跡は20mぶんの長さを持つ*
ので、事件の大きさが時間ではなく**幅**で伝わる。
...
6色1組のパレットを**全種族で固定**する。種族は見た目だけの区分ではなく
編成の材料（2.9）なので、色相だけでなく**形の記号**も種族ごとに固定する ――
...
```text
48×48ドットのドット絵を1枚。…真横から見た側面図で、キャラは画面の右を向く。…
...
**症状。** 20mは画面で78px ―― *キャラの絵（96px幅）より狭い*。時間としては
巨兵で7.1秒と長いのに、**移動そのものは自分の体の幅ぶんも無い**（2.11）。
硬直中は判定が消える（殴られず、すり抜けられる）のに、それも出ていなかった。

**直したのは、距離を見せようとするのをやめたこと。** 78pxをどう強調しても
78pxは78pxなので、代わりに**残るもの**を描く ―― 3つとも `Battle` を読むだけで、
View はフレームをまたぐ状態を持たない（起点は `x + 向き × 後退距離` で出る。
硬直中は動かないので、この値は硬直のあいだ固定される）。

| 出すもの | 何を言っているか |
| --- | --- |
| **水色の跡＋起点の縦棒** | 「いま下がった」。**跡の長さがそのまま後退した20m** ―― 移動は78pxでも、幅は残る |
...
| 出撃できる種類 | `--unit` で自由 | 試遊用の8種（`preset_roster.json`） |
| 相手 | 3方針 ＋ `--enemy-roster full` | 3方針（持ち物は常に同じ8種） |
